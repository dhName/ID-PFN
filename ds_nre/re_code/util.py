    ...
    for dataset in datasets:
        for data in dataset:
            for entity in data['entityMentions']:
                type_vocab.add('B-'+entity['label'])
                type_vocab.add('I-'+entity['label'])
                type_vocab.add('E-'+entity['label'])
                type_vocab.add('S-'+entity['label'])
...
def get_types(sentence, entity_to_type, type_dict):
    ...
    for word in sentence:
        if word[:7] == '<START>' and word[-5:] == '<END>':
            types.append('S-'+entity_to_type[word])
        elif word[:7] == '<START>':
            type_ = entity_to_type[word]
            types.append('B-'+type_)
        elif word[-5:] == '<END>':
            types.append('E-'+type_)
            type_ = 'None'
        else:
            if type_ == 'None':
                types.append(type_)
...
def get_logger(mode):
    ...
    handler2 = logging.FileHandler(filename=mode+".log")
...
def check_env(config):
    if not os.path.exists("models/"+config["model_name"]+'/ckpt_model'):
        os.makedirs("models/"+config["model_name"]+'/ckpt_model')
    if not os.path.exists("models/"+config["model_name"]+'/best_model'):
        os.makedirs("models/"+config["model_name"]+'/best_model')
...
def build_maps(train_datas, config, logger):
    # build maps file
    if "model_name" in config.keys() and os.path.exists("models/"+config["model_name"]+'maps.npy'):
        vocab, type_dict, label_dict = np.load("models/"+config["model_name"]+'/maps.npy')
    else:
        vocab = get_vocab([train_datas])
        type_dict = get_typeDict([train_datas])
        label_dict = get_labelDict([train_datas],  config['sel_label'])

        if config['pretrained_word']:
            vocab=json.load(codecs.open(r"E:\PycharmProjects\my_graduation_project\word_vec\word2id.json", 'r', encoding='utf-8'))
        if "model_name" in config.keys():
            np.save("models/"+config["model_name"]+'/maps.npy', [vocab, type_dict, label_dict])

    if "model_name" in config.keys():
        with open("models/"+config["model_name"]+'/config_file', 'w', encoding='utf8') as w:
            json.dump(config, w)
